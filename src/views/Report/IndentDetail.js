...
const IndentDetail = () => {
  ...
  // Mongo ObjectIds are 24-char hex strings. Anything else (e.g. a
  // center/product NAME passed in the URL) is not safe to send to the API
  // as centerId/productId — it would trigger a CastError ("Invalid ID format")
  // on the backend.
  const isValidObjectId = (value) => /^[0-9a-fA-F]{24}$/.test(value || '');

  // Resolves a value that may be an id OR a display name into a real
  // Mongo _id, using the already-fetched centers/products lists.
  const resolveIdFromNameOrId = (value, list, idField, nameField) => {
    if (!value) return '';
    if (isValidObjectId(value)) return value;

    const decoded = decodeURIComponent(value).trim().toLowerCase();
    const match = list.find(
      (item) => (item[nameField] || '').trim().toLowerCase() === decoded
    );
    return match ? match[idField] : '';
  };
  ...
  useEffect(() => {
    const initializeData = async () => {
      try {
        ...
        const [fetchedCenters, fetchedProducts] = await Promise.all([fetchCenters(), fetchProducts()]);
        
        const hasUrlFilters = urlParams.product || urlParams.center || urlParams.productId || urlParams.centerId;
        
        let searchParams = {};
        
        if (hasUrlFilters) {
          // Prefer an explicit id if one was passed; otherwise resolve the
          // (possibly name-based) `center`/`product` params against the
          // fetched lists so we never send a name to the backend as an id.
          const resolvedCenter =
            (urlParams.centerId && isValidObjectId(urlParams.centerId) && urlParams.centerId) ||
            resolveIdFromNameOrId(urlParams.center, fetchedCenters || [], '_id', 'centerName');

          const resolvedProduct =
            (urlParams.productId && isValidObjectId(urlParams.productId) && urlParams.productId) ||
            resolveIdFromNameOrId(urlParams.product, fetchedProducts || [], '_id', 'productTitle');

          if (urlParams.center && !resolvedCenter) {
            console.warn(`⚠️ Could not resolve center "${urlParams.center}" to an id; ignoring center filter.`);
          }
          if (urlParams.product && !resolvedProduct) {
            console.warn(`⚠️ Could not resolve product "${urlParams.product}" to an id; ignoring product filter.`);
          }
