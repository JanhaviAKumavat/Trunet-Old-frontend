...
const AddReseller = () => {
  ...
  const fetchVendor = async (centerId) => {
    try {
      const res = await axiosInstance.get(`/resellers/${centerId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id) {
        await axiosInstance.put(`/resellers/${id}`, formData);
        setAlert({ type: 'success', message: 'Data updated successfully!' })
      } else {
        await axiosInstance.post('/resellers', formData);
        setAlert({ type: 'success', message: 'Data added successfully!' })
      }
      setTimeout(() =>navigate('/reseller-list'),1500);
    } catch (error) {
  ...
  return (
    <div className="form-container">
      <div className="title">{id ? 'Edit' : 'Add'} Reseller</div>
